...
// /
func (s *RiskCtrlRule) GetContractAbiBriefs(ctx context.Context, ChainId string, kind string) ([]*entity.Contractabi, error) {
	// model := g.Model(dao.Contractabi.Table()).Ctx(ctx).Cache(gdb.CacheOption{
	model := dao.Contractabi.Ctx(ctx).Cache(gdb.CacheOption{
		Duration: s.dur,
		Name:     dao.Contractabi.Table() + ChainId + kind,
		Force:    true,
	}).Fields(
...
func (s *RiskCtrlRule) GetContractAbi(ctx context.Context, ChainId string, address string, flush bool) (*entity.Contractabi, error) {
	rst, err := dao.Contractabi.Ctx(ctx).Cache(gdb.CacheOption{
		...
		Name:  dao.Contractabi.Table() + ChainId + address,
		Force: true,
	}).
...
func (s *RiskCtrlRule) GetContractRuleBriefs(ctx context.Context, ChainId string, kind string) ([]*entity.Contractrule, error) {
	model := dao.Contractrule.Ctx(ctx).Cache(gdb.CacheOption{
		Duration: s.dur,
		Name:     dao.Contractrule.Table() + ChainId + kind,
		Force:    true,
	}).Fields(
...
// /
func (s *RiskCtrlRule) GetContractRule(ctx context.Context, ChainId string, address string, flush bool) (*entity.Contractrule, error) {
